      ...
      >
          ...
          >
            <div className="text-center">
              <div className="flex justify-center space-x-6">
                <div className={`w-4 h-4 rounded-full ${step >= 1 ? '#4A4947' : '#D8D2C2'}`} style={{ backgroundColor: step >= 1 ? '#4A4947' : '#D8D2C2' }}></div>
                <div className={`w-4 h-4 rounded-full ${step >= 2 ? '#4A4947' : '#D8D2C2'}`} style={{ backgroundColor: step >= 2 ? '#4A4947' : '#D8D2C2' }}></div>
                <div className={`w-4 h-4 rounded-full ${step >= 3 ? '#4A4947' : '#D8D2C2'}`} style={{ backgroundColor: step >= 3 ? '#4A4947' : '#D8D2C2' }}></div>
              </div>
              <p className="text-xs font-semibold mt-1" style={{ color: "#4A4947" }}>Step {step} of 3</p>
            </div>
          </motion.div>
        </div>
        ...
        >
          <div className="relative h-48" style={{ backgroundColor: "#4A4947" }}>
            <div className="absolute inset-0 opacity-20" style={{ 
              backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23D8D2C2' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")",
              backgroundSize: "30px 30px"
            }}></div>
            <div className="absolute inset-0 flex items-center justify-center">
              <h2 className="text-4xl font-bold text-white z-10 text-center">Become a Provider</h2>
            </div>
          </div>
          ...
          <div className="p-10">
            {step === 1 && (
              <motion.div
                ...
                exit="exit"
              >
                <h3 className="text-2xl font-semibold mb-8" style={{ color: "#4A4947" }}>Personal Information</h3>
                {user && (
                  <div className="mb-10 p-6 rounded-lg" style={{ backgroundColor: "#D8D2C2" }}>
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        <p className="text-sm" style={{ color: "#4A4947", opacity: 0.7 }}>Name</p>
                        <p className="font-semibold text-lg" style={{ color: "#4A4947" }}>{user.name}</p>
                      </div>
                      <div>
                        <p className="text-sm" style={{ color: "#4A4947", opacity: 0.7 }}>Email</p>
                        <p className="font-semibold text-lg" style={{ color: "#4A4947" }}>{user.email}</p>
                      </div>
                      <div>
                        <p className="text-sm" style={{ color: "#4A4947", opacity: 0.7 }}>Phone</p>
                        <p className="font-semibold text-lg" style={{ color: "#4A4947" }}>{user.phone}</p>
                      </div>
                      <div>
                        <p className="text-sm" style={{ color: "#4A4947", opacity: 0.7 }}>Address</p>
                        <p className="font-semibold text-lg" style={{ color: "#4A4947" }}>{user.address}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {step === 2 && (
              <motion.div
                ...
                exit="exit"
              >
                <h3 className="text-2xl font-semibold mb-8" style={{ color: "#4A4947" }}>Upload Documents</h3>
                <form onSubmit={handleSubmit(onSubmit)}>
                  <div className="mb-8">
                    <label className="block mb-3 text-base font-medium" style={{ color: "#4A4947" }}>Profile Image</label>
                    <div className="flex items-center space-x-6">
                      ...
                      >
                        {previewImage ? (
                          ...
                          <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 20 20" style={{ color: "#4A4947" }}>
                            <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                          </svg>
                        )}
                      </motion.div>
                      <div className="flex-1">
                        ...
                        >
                          <input 
                            ...
                            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                          />
                          <p className="text-base" style={{ color: "#4A4947" }}>Drop your image here or click to browse</p>
                          <p className="text-sm mt-2" style={{ color: "#4A4947", opacity: 0.7 }}>JPG, PNG or GIF (max 2MB)</p>
                        </motion.div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-8">
                    <label className="block mb-3 text-base font-medium" style={{ color: "#4A4947" }}>Identity Document</label>
                    <motion.div 
                    ...
                    >
                      ...
                      {documentImage ? (
                        <div className="flex items-center justify-center h-full">
                          <p className="text-base font-medium" style={{ color: "#4A4947" }}>Document uploaded</p>
                          <span className="ml-2" style={{ color: "#4A4947" }}>✓</span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center h-full">
                          <svg className="w-12 h-12 mb-3" fill="currentColor" viewBox="0 0 20 20" style={{ color: "#4A4947", opacity: 0.7 }}>
                            <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clipRule="evenodd" />
                          </svg>
                          <p className="text-base" style={{ color: "#4A4947" }}>Upload your identity document</p>
                          <p className="text-sm mt-2" style={{ color: "#4A4947", opacity: 0.7 }}>PDF, JPG or PNG (max 5MB)</p>
                        </div>
                      )}
                    </motion.div>
                  </div>
                  ...
                  <div className="flex space-x-4">
                    ...
                    >
                      {loading ? (
                        <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: "#FAF7F0", borderTopColor: "transparent" }}></div>
                      ) : (
                        "Submit Application"
                      )}
                    </motion.button>
            ...
            {step === 3 && (
              ...
              >
                <motion.div
                  ...
                  style={{ backgroundColor: "#D8D2C2" }}
                >
                  <svg className="w-14 h-14" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "#4A4947" }}>
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                  </svg>
                </motion.div>
                <h3 className="text-2xl font-bold mb-3" style={{ color: "#4A4947" }}>Application Submitted!</h3>
                <p className="text-lg mb-8" style={{ color: "#4A4947", opacity: 0.8 }}>{message || "Your application has been received. We'll review it soon."}</p>
                <motion.button
                  onClick={() => window.location.href = "/dashboard"}
        ...
        <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 z-10">
          <motion.div 
            ...
            whileHover={{ y: -2 }}
          >
            <p className="text-sm font-medium" style={{ color: "#4A4947" }}>Need help? Contact support</p>
          </motion.div>
        </div>
      </motion.div>
    </div>
